...
if ! grep -q "sub_filter_once" /etc/nginx/nginx.conf; then
  sed -i '/location \/ {/a \            sub_filter "</body>" "<script>if((window.location.pathname===\"/\" || window.location.pathname===\"/auth\") && !localStorage.getItem(\"planna_banner_dismissed\")){document.write(\"<div id=\\\"planna-banner\\\" style=\\\"position:fixed;bottom:0;width:100%;background:#2563EB;color:white;text-align:center;padding:15px;z-index:9999;font-weight:bold;font-size:14px;\\\">Planna is a powerful social media scheduling application. By connecting your accounts, you authorize Planna to publish posts and view analytics on your behalf.<span style=\\\"position:absolute;right:20px;top:50%;transform:translateY(-50%);cursor:pointer;font-size:18px;\\\" onclick=\\\"document.getElementById('planna-banner').style.display='none';localStorage.setItem('planna_banner_dismissed','true');\\\">&times;</span></div>\");}</script></body>";\n            sub_filter_once on;' /etc/nginx/nginx.conf
fi
